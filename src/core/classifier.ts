  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (text === 'completed' || text === 'task-pending') {
        const result: TaskCategory = text
...
/** 关键词预检：不依赖 LLM，快速判定 */
function classifyLocal(message: string, reply: string, _toolNames: string[]): TaskCategory | null {
  // 高风险命令检查（优先于其他规则）
  for (const pattern of HIGH_RISK_COMMANDS) {
    if (pattern.test(message)) return 'task-pending'
  }
  if (PENDING_RE.test(message)) return 'task-pending'
  if (COMPLETED_RE.test(reply)) return 'completed'
  return null
}

/** LLM 调用失败时的降级规则 */
function fallback(message: string, reply: string, toolNames: string[]): TaskCategory {
  // 工具调用多 → 大概率已完成
  if (toolNames.length > 2) return 'completed'
  // 回复暗示后续操作
  if (PENDING_FALLBACK_RE.test(reply)) return 'task-pending'
  return 'reference'
}
