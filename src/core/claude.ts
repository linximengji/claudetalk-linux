...
export function saveSessionMap(workDir: string, sessionMap: Map<string, SessionEntry>): void {
  const sessionFile = getSessionFile(workDir)
  try {
...
export function getSessionMap(workDir: string): Map<string, SessionEntry> {
...
function consumeClearedFlag(sessionKey: string, existingSessionId: string | undefined): boolean {
  if (!existingSessionId) return false
...
async function syncCompactSession(
  ...
  return new Promise<string | undefined>((resolve) => {
    ...
    child.on('close', (code: number | null) => {
      ...
      if (lastJson) {
        try {
          const r = JSON.parse(lastJson) as ClaudeResponse
          if (r.session_id) { resolve(r.session_id); return }
        } catch { /* ignore */ }
      }
      resolve(undefined)
    })
  ...
  const summary = await new Promise<string>((resolve) => {
    ...
    child.on('close', () => {
      ...
      if (lastJson) {
        try {
          const r = JSON.parse(lastJson) as ClaudeResponse
          if (r.result) { resolve(r.result); return }
        } catch { /* ignore */ }
      }
...
async function preProcessSessionCheck(
  ...
  if (!existingEntry?.sessionId) return
...
function buildClaudeArgs(options: CallClaudeOptions): {
  args: string[]
  actualMessage: string
  sessionKey: string
  sessionMap: Map<string, SessionEntry>
  existingSessionId: string | undefined
  currentSubagentEnabled: boolean
  currentConfig: ClaudeTalkConfig | null
} {
  const {
    message,
    conversationId,
 ...
 */
async function _execClaude(options: CallClaudeOptions, retryCount = 0): Promise<string> {
  const {
    message,
    conversationId,
    workDir,
  ...
  let { args, actualMessage, sessionKey, sessionMap, existingSessionId, currentSubagentEnabled, currentConfig } =
    ...
    child.on('close', (code: number | null) => {
      ...
      try {
        ...
        if (!consumeClearedFlag(sessionKey, existingSessionId)) {
          ...
          const prevCumulated = (entry as any).cumulatedInputTokens ?? 0
          const prevToolCalls = (entry as any).toolCallCount ?? 0
          const newSessionId: string = response.session_id || existingSessionId || ''
          sessionMap.set(sessionKey, {
            ...
            sessionSummary: (entry as any).sessionSummary,
 ...
 */
async function _execClaudeStreaming(
  options: CallClaudeOptions,
  onEvent: (event: StreamEvent) => void,
  retryCount = 0,
): Promise<{ sessionId: string; result: string }> {
    ...
    child.stdout.on("data", (data: Buffer) => {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (event.type === 'system') continue // 跳过 system 事件
          ...
          if (event.type === 'assistant' && event.message?.content) {
            ...
            // token 日志
            const muValues: Array<{inputTokens?: number; outputTokens?: number; cacheReadInputTokens?: number; costUSD?: number}> =
              event.modelUsage ? Object.values(event.modelUsage) : []
            appendTokenLog(workDir, {
              ts: new Date().toISOString(),
              workDir,
            ...
            if (!consumeClearedFlag(sessionKey, existingSessionId)) {
              const streamMuValues: Array<{inputTokens?: number}> =
                event.modelUsage ? Object.values(event.modelUsage) : []
              ...
              const newStreamSessionId: string = finalSessionId || existingSessionId || ''
              sessionMap.set(sessionKey, {
