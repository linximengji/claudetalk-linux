...
export async function startBot(options: StartBotOptions): Promise<void> {
  ...
  const activeTrips = new Map<string, { tripId: string; sessionId: string }>()
  ...
  function isTripIntent(text: string): boolean {
    const lower = text.toLowerCase()
    return TRIP_TRIGGERS.some(t => lower.includes(t))
  }

  function isTripEndIntent(text: string): boolean {
    const lower = text.toLowerCase()
          ...
          const { result: finalResult } = await callClaudeStreaming(
            ...
            async (event: StreamEvent) => {
              if (event.type === 'text' && event.text) {
                try { await channel.editMessage!(context.conversationId, statusMsgId, event.text) } catch {}
              }
            }
          )
