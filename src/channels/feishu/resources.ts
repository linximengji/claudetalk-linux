...
export async function downloadImage(
  ...
  try {
    ...
    const response = await fetch(
      `${apiBase}/im/v1/messages/${messageId}/resources/${imageKey}?type=image`,
...
export async function downloadFile(
  ...
  try {
    ...
    const response = await fetch(
      `${apiBase}/im/v1/messages/${messageId}/resources/${fileKey}?type=file`,
      { headers: { Authorization: `Bearer ${accessToken}` } },
    )
    if (!response.ok) return null
    ...
    const arrayBuffer = await response.arrayBuffer()
    if (arrayBuffer.byteLength > 50 * 1024 * 1024) return null
...
export async function recognizeSpeech(
  ...
  try {
    const response = await fetch(
      `${apiBase}/speech_to_text/v1/speech/recognize`,
      {
        method: 'POST',
        headers: {
