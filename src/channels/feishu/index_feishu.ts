...
import { loadConfig, callClaude } from '../../core/claude.js';
import { archiveConversation, writeTaskToIndex } from '../../core/phone-archive.js';
import { handleApprovalCallback, buildArchiveConfirmCard, buildTaskConfirmCard } from './approval-handler.js';
import {
  loadPeerMessages,
  removePeerMessages,
...
import { FeishuApiClient, FEISHU_API_BASE, parseFeishuMessage, SUPPORTED_MSG_TYPES } from '../../feishu-shared/index.js';
...
export class FeishuClient implements Channel {
  ...
  private processedEventIds = new Map<string, number>(); // event id → timestamp, for dedup cleanup timer
  private readonly DEDUP_TTL_MS = 24 * 60 * 60 * 1000;
  private _dedupCleanupTimer: ReturnType<typeof setInterval> | null = null;
  private _isBusy = false; // 正在处理消息，后续消息进排队提示
  private _peerProcessing = false; // processPeerMessages 排他锁，防止 setInterval 重入导致并发处理
  ...
  private processedOutboxIds = new Set<string>();

  // 群成员管理（委托给独立模块）
  private readonly chatMemberStore: ChatMemberStore;
  private readonly chatMemberResolver: ChatMemberResolver;
  ...
  constructor(config: FeishuChannelConfig) {
    ...
    this.chatMemberResolver = new ChatMemberResolver(
      this.chatMemberStore,
      FEISHU_API_BASE,
      () => this.getAccessToken(),
      this.logger,
    );
  ...
  private startOutboxPolling(): void {
    if (this.config.profileName !== 'twin') return;
  ...
  private async processOutbox(outboxPath: string): Promise<void> {
    if (!fs.existsSync(outboxPath)) return;
    const lines = fs.readFileSync(outboxPath, 'utf-8').split('\n').filter((l) => l.trim());
    const pending: Array<{ id: string; receive_id: string; content: string }> = [];
  ...
  private async processPeerMessages(botName: string): Promise<void> {
    ...
    if (this._peerProcessing) return;
    ...
    try {
      ...
      const pendingMessages = messages.filter(
        (msg: PeerMessage) => !this.processedPeerIds.has(msg.id)
      );

      // 二次去重：同一飞书 messageId 只处理一次（防 WS 重复投递）
      const seenMsgIds = new Set<string>()
      const dedupedMessages = pendingMessages.filter(msg => {
        if (seenMsgIds.has(msg.messageId)) return false
      ...
      for (const [idx, peerMsg] of dedupedMessages.entries()) {
        ...
        if (peerMsg.message === '__confirm_archive__') {
          ...
          if (pair) {
            this.execConfirmArchive(peerMsg.chatId, pair).catch((err) =>
              this.logger(`${traceTag}__confirm_archive__ error: ${err}`)
            );
          } else {
      ...
      if (succeededIds.length > 0) {
        ...
        for (const id of succeededIds) this.processedPeerIds.set(id, now);
      ...
      for (const [id, ts] of this.processedPeerIds.entries()) {
        if (now - ts > 60 * 60 * 1000) this.processedPeerIds.delete(id)
      }
    } finally {
   ...
   */
  setLastConvGetter(getter: (convId: string) => { message: string; reply: string } | null): void {
            ...
            try {
              ...
              const memoSummary = (() => {
                try {
                  ...
                  if (p.ingestion) return '✓ 记忆已记录';
                  return `✓ ${p.stored || '?'}条记忆`;
                } catch { return '缺口已关闭，已记录'; }
              })();
...
try:
   ...
   */
  private shouldAskGap(userMessage: string): Promise<{should_ask?: boolean; question?: string}> {
...
try:
    ...
    print('{}')`,
      ...
      let out = '';
      python.stdout?.on('data', (d: Buffer) => { out += d.toString(); });
      python.on('close', () => {
            ...
            downloadImage(parsed.image_key, messageId, FEISHU_API_BASE, this.claudetalkDir, token)
              .then(localPath => {
                if (localPath) {
                  ...
                  const idx = msgs.findIndex((m: any) => m.messageId === messageId);
                  if (idx >= 0) {
   ...
   */
  private handleCardAction(data: Record<string, unknown>): { toast: { type: string; content: string }; card?: unknown } {
    ...
    const cardToken = (data.token as string) || '';
    ...
    switch (actionType) {
      ...
      case 'confirm-archive': {
        ...
        if (!pair) return { toast: { type: 'warning', content: '没有找到最近对话' } };
      ...
      case 'confirm-task-create': {
        ...
        const taskType = (value?.type as string) || 'task';
        const priority = (value?.priority as string) || 'medium';
        const progressNotes = (value?.progress_notes as string) || '';
   ...
   */
  private async execConfirmArchive(chatId: string, pair: { message: string; reply: string }): Promise<void> {
    try {
   ...
   */
  private async updateCardToDone(messageId: string, taskId: string, value: Record<string, unknown>): Promise<void> {
    const token = await this.getAccessToken();
    const summary = (value?.task_summary as string) || taskId;
  ...
  private async updateCardToCreated(messageId: string, taskId: string, summary: string): Promise<void> {
    const token = await this.getAccessToken();
  ...
  private async sendAndWritePeerMessage(conversationId: string, content: string, isGroup: boolean): Promise<string> {
  ...
  async sendMessage(conversationId: string, content: string, isGroup: boolean): Promise<void> {
    await this.sendAndWritePeerMessage(conversationId, content, isGroup);
  }

  async sendMessageWithId(conversationId: string, content: string, isGroup: boolean): Promise<string> {
   ...
   */
  async sendCard(receiverId: string, cardBody: string, isGroup: boolean): Promise<void> {
    const accessToken = await this.getAccessToken();
    const receiveIdType = receiverId.startsWith('ou_') ? 'open_id' : 'chat_id';
    void isGroup;
   ...
   */
  async sendMarkdownCard(receiverId: string, markdown: string, isGroup: boolean): Promise<void> {
    const accessToken = await this.getAccessToken();
    const receiveIdType = receiverId.startsWith('ou_') ? 'open_id' : 'chat_id';
    void isGroup;
