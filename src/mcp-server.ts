...
function loadFeishuConfigLocal(workDir: string): FeishuConfig | null {
...
function buildCardPayload(args: any): object | null {
  try {
    const elements: any[] = []
    if (args.content) elements.push({ tag: 'markdown', content: args.content })

    if (args.buttons?.length) {
      const actions = args.buttons.map((b: any) => {
        const btn: any = {
          tag: 'button',
          text: { tag: 'plain_text', content: b.text },
          value: b.value,
        }
        if (b.type) btn.type = b.type
...
function handleToolCall(name: string, args: any, api: FeishuApiClient, cfg: FeishuConfig): Promise<any> {
  switch (name) {
    case 'feishu_send_text': {
      ...
      if (!receiveId) return Promise.resolve({ error: 'No receive_id. Set FEISHU_RECEIVE_ID env var or pass receive_id argument.' })
    ...
    case 'feishu_send_card': {
      ...
      if (!card) return Promise.resolve({ error: 'Failed to build card payload' })
      return (async () => {
        const raw = await api.sendCard(receiveId, JSON.stringify(card), receiveIdType)
        if (raw.code !== 0) return { error: `Failed to send card: ${raw.msg}` }
    ...
    case 'feishu_send_image': {
      ...
      const receiveIdType = args.receive_id_type || cfg.defaultReceiveIdType
      if (!receiveId) return Promise.resolve({ error: 'No receive_id. Set FEISHU_RECEIVE_ID env var or pass receive_id argument.' })
      if (!args.file_path && !args.image_key) return Promise.resolve({ error: 'Either file_path or image_key is required.' })
      return (async () => {
    ...
    case 'feishu_send_file': {
      const receiveId = args.receive_id || cfg.defaultReceiveId
      const receiveIdType = args.receive_id_type || cfg.defaultReceiveIdType
      if (!receiveId) return Promise.resolve({ error: 'No receive_id.' })
      if (!args.file_path && !args.file_key) return Promise.resolve({ error: 'Either file_path or file_key is required.' })
      return (async () => {
...
async function listenWithRetry(server: http.Server, port: number, maxRetries = 8, delayMs = 2000): Promise<boolean> {
