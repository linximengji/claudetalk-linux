 ...
 */
function parseAutoConfigItems(data) {
    return data.map((item) => {
        const agentKey = Object.keys(item).find((k) => k !== 'prompt');
        const meta = item[agentKey];
 ...
 */
function promptMultiLineInput(question) {
    const readline = createInterface({ input: process.stdin, output: process.stdout });
    return new Promise((resolve) => {
...
function saveRawConfig(config, filePath) {
 ...
 */
function getClaudeMdTemplatePath() {
 ...
 */
function isClaudeMdContentAlreadyPresent(targetClaudeMdPath, templateContent) {
    if (!existsSync(targetClaudeMdPath))
        return false;
 ...
 */
async function setupClaudeMd(workDir) {
 ...
 */
async function autoSetup(workDir) {
    ...
    try {
        ...
        autoConfigData = JSON.parse(rawContent);
    }
    catch (error) {
 ...
 */
async function editSetup(workDir) {
    const targetFile = join(workDir, LOCAL_CONFIG_FILENAME);
...
// ========== 交互式配置向导 ==========
async function interactiveSetup(workDir, profile) {
    const targetFile = join(workDir, LOCAL_CONFIG_FILENAME);
    ...
    const allChannels = getAllChannelDescriptors();
    const existingChannel = existingProfile?.channel;
    console.log('📡 消息通道选择:');
    allChannels.forEach((ch, index) => {
        console.log(`   ${index + 1}. ${ch.type.padEnd(10)} - ${ch.label}`);
    });
    let channelType;
    while (true) {
        const channelInput = await promptInput(existingChannel ? `请选择 (1-${allChannels.length}) [${existingChannel}]: ` : `请选择 (1-${allChannels.length}): `);
        const channelIndexInput = parseInt(channelInput, 10);
        if (!isNaN(channelIndexInput) && channelIndexInput >= 1 && channelIndexInput <= allChannels.length) {
            channelType = allChannels[channelIndexInput - 1].type;
            break;
    ...
    const descriptor = getChannelDescriptor(channelType);
    const existingChannelConfig = (existingProfile?.[channelType] ?? {});
    const channelConfig = {};
    console.log('');
    console.log(`🔑 ${descriptor.label}配置`);
    if (descriptor.configFields.some(f => f.hint)) {
 ...
 * 创建 SubAgent 配置文件
 */
async function createSubagentFile(profileName, workDir, systemPrompt, model) {
...
`);
    ...
    if (profile) {
        process.title = `claudetalk-${profile}`;
        console.log('');
    ...
    const startPromises = profileNames.map((profileName, index) => new Promise((resolve) => {
        setTimeout(() => {
