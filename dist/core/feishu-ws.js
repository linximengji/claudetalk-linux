...
export class FeishuWsClient {
    ...
    constructor(options) {
        ...
        this.log = options.logger || ((msg) => { console.error(`[${this.label}] ${msg}`); });
    }
    /** 注册事件处理器 */
    ...
    /** 分片合并缓存 */
    _mergeFragments(messageId, sum, seq, data, traceId) {
        ...
        if (!entry) {
            ...
            const buffer = new Array(sum).fill(null);
            buffer[seq] = data;
    ...
    /** 发送 ACK */
    _sendAck(headers, service, seqId, logId) {
        const ws = this.ws;
        ...
        ws.send(frame, (err) => {
            if (err)
                this.log(`ACK send error: ${err.message}`);
        });
    }
    /** Protobuf 编码 */
    _encodeFrame(headers, method, service, payload, seqId, logId) {
        const parts = [];
    ...
    _sendPong(service) {
        ...
        if (!ws || ws.readyState !== WebSocket.OPEN)
            return;
        const frame = this._encodeFrame([{ key: HEADER_KEY_TYPE, value: MESSAGE_TYPE_PONG }], FRAME_CONTROL, service, Buffer.alloc(0), 0, 0);
        ws.send(frame, (err) => {
            if (err)
                this.log(`Pong send error: ${err.message}`);
        });
    }
    /** Ping 循环 */
    _startPingLoop() {
        ...
        this._pingTimer = setInterval(() => {
            ...
            if (!ws || ws.readyState !== WebSocket.OPEN)
                return;
            const frame = this._encodeFrame([{ key: HEADER_KEY_TYPE, value: MESSAGE_TYPE_PING }], FRAME_CONTROL, parseInt(this.serviceId, 10) || 0, Buffer.alloc(0), 0, 0);
            ws.send(frame, (err) => {
                if (err)
                    this.log(`Ping send error: ${err.message}`);
            });
    ...
    /** 分发事件给注册的处理器 */
    async _dispatchEvent(eventType, data, traceId) {
        const handlers = this.handlers.get(eventType) || [];
