 ...
 * @param workDir - 工作目录
 */
export function initLogFile(workDir) {
...
function rotateLogIfNeeded() {
    ...
    const now = Date.now();
    if (now - _lastRotateCheck < ROTATE_INTERVAL_MS)
        return;
    _lastRotateCheck = now;
    const todayStr = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}-${String(new Date().getDate()).padStart(2, '0')}`;
    if (logFilePath.includes(todayStr))
        return; // 日期一致，无需切换
    // 跨日：关旧流、开新文件
    logFileStream.end();
    logFileStream = null;
 ...
 */
export function createLogger(channel, profile) {
    let prefix = '';
    if (channel && profile) {
