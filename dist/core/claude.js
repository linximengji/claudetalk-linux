...
function loadConfigFromFile(filePath, profile) {
    ...
    try {
        ...
        if (profile && !raw.profiles?.[profile]) {
            ...
            const config = { channel: 'feishu', subagentEnabled: true };
 ...
 */
function rotateFileSync(filePath, maxKept, _dir, _baseName) {
    // 删除最旧的备份
    try {
        renameSync(`${filePath}.${maxKept}`, `${filePath}.${maxKept + 1}`);
    }
    catch { /* not exists */ }
        ...
        child.on('close', (code) => {
            ...
            if (code !== 0) {
                ...
                if (_draining || (code === null && stderr.length === 0)) {
                    if (_draining)
                        logger(`[claude] Skip retry: draining`);
        ...
        child.on('close', (code) => {
            ...
            if (code !== 0) {
                ...
                if (_draining || (code === null && stderr.length === 0)) {
                    ...
                    if (_draining)
                        logger(`[claude-stream] Skip retry: draining`);
                    reject(new Error(`claude subprocess killed (code=${code})`));
                    return;
                }
                if (retryCount < MAX_RETRY_COUNT) {
