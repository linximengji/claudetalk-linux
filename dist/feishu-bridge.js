...
function isGroupChat(chatId, chatType) {
    if (chatType === 'group')
        return true;
    if (chatType === 'p2p')
        return false;
...
function receiptTypeForChat(chatId, item) {
...
async function pollMessages(api, claudeTalkDir, chatIds, botAppName) {
    ...
    for (const chatId of chatIds) {
        ...
        const nonBotItems = items.filter((i) => !KNOWN_BOT_APP_IDS.has(i.sender?.id) && !_seenMessageIds.get(chatId)?.has(i.message_id));
                ...
                if (rootId) {
                    ...
                    try {
                        if (fs.existsSync(gapStatePath)) {
                            ...
                            const gaps = gs.gaps || [];
                            const match = gaps.find((g) => g.message_id === rootId && !g.answered);
                            if (match) {
                    ...
                    try {
                        ...
                        try {
                            if (fs.existsSync(gapStatePath)) {
                                ...
                                const sorted = [...gaps].filter((g) => !g.answered && g.pushed_at)
                        ...
                        const receiptType = receiptTypeForChat(chatId, item);
                        api.sendText(chatId, replyText, receiptType).catch((e2) => {
...
async function main() {
    ...
    let chatIds = ['oc_44ba0d81afa9189a67ef99d0bce1124d'];
    try {
        if (fs.existsSync(configPath)) {
            ...
            for (const [name, p] of Object.entries(profiles)) {
                ...
                if (id)
                    ...
                    console.error(`[feishu-bridge] profile ${name}: no FEISHU_RECEIVE_ID, chatIds may be incomplete`);
            }
