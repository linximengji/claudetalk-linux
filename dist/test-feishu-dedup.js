...
function shouldSkipBotSelf(sender, botOpenId) {
...
function shouldDeduplicateByMessageId(messageId, processedEventIds, dedupTtl) {
    const now = Date.now();
    if (messageId && processedEventIds.has(messageId) && now - processedEventIds.get(messageId) < dedupTtl)
        return true;
    if (messageId)
...
function shouldProcessGroupMessage(message) {
    if (message.chat_type !== 'group')
        return true; // p2p 不做检查
...
function runAllTests() {
    ...
    const p2pMsg = { message_id: 'M5', chat_type: 'p2p', chat_id: 'c1', message_type: 'text', content: '{}', mentions: [] };
    assert(shouldProcessGroupMessage(p2pMsg) === true, 'p2p 消息不需要 @mention');
    // 3b: 群聊有 @mention → 通过
    const groupWithMention = { message_id: 'M6', chat_type: 'group', chat_id: 'c2', message_type: 'text', content: '{}', mentions: [{ id: { open_id: USER_OPEN_ID }, name: 'user' }] };
    assert(shouldProcessGroupMessage(groupWithMention) === true, '群聊有 @mention → 通过');
    // 3c: 群聊无 @mention → 跳过
    const groupNoMention = { message_id: 'M7', chat_type: 'group', chat_id: 'c2', message_type: 'text', content: '{}' };
...
runAllTests();
//# sourceMappingURL=test-feishu-dedup.js.map