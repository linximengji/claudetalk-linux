        ...
        try {
            ...
            if (useStreaming) {
                ...
                if (profile === 'twin') {
                    ...
                    const pendingTwinMsg = context.processedMessage ?? message;
                    try {
                        // 流式：思考过程先露出，随后正式回答逐字推送
                        let thoughtBuf = '';
                        finalResult = await callTwinChatStream(pendingTwinMsg, caller, chatContext, evt => {
                        ...
                        try {
                            finalResult = await callTwinChat(pendingTwinMsg, caller, chatContext);
                        }
                        catch (err2) {
