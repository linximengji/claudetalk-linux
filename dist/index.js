...
function httpRequestCheck(path, port) {
    return new Promise(resolve => {
 ...
 */
function createChannel(channelType, config, workDir, profileName) {
    ...
    for (const field of descriptor.configFields) {
        if (field.required && !channelConfig[field.key]) {
            throw new Error(`${channelType} 配置缺失字段 "${field.key}"，请在 profile.${channelType}.${field.key} 中填写`);
        }
    }
    // 将 profile 级别的通用字段注入到 channelConfig，供 Channel 实现使用
    const enrichedChannelConfig = {
        ...channelConfig,
...
export async function startBot(options) {
    ...
    try {
        ...
        appendFileSync(join(markerDir, 'claudetalk.log'), `[${new Date().toISOString()}] [BOOT] argv="${process.argv.slice(1).join(' ')}" workDir=${workDir}\n`, 'utf-8');
    }
    catch { /* best-effort */ }
    // 初始化日志文件
    initLogFile(workDir);
    ...
    function _writeDaemonCrashMarker() {
        try {
            writeFileSync(daemonCrashMarker, String(Date.now() / 1000), 'utf-8');
        }
        catch { }
    }
    /** Track startup progress at key phases */
    const _startupPhases = [];
    ...
    function _writeExitMarker(type, extra) {
        ...
        try {
            ...
            if (isCrash)
                _writeDaemonCrashMarker();
            appendFileSync(join(crashMarkerDir, `claudetalk-${new Date().toISOString().slice(0, 10)}.log`), `[EXIT] ${JSON.stringify(marker)}\n`, 'utf-8');
        }
        catch {
            try {
                writeFileSync(join(crashMarkerDir, 'exit-marker.txt'), `${type} ${marker.ts} pid=${marker.pid}\n`, 'utf-8');
            }
            catch { }
        }
    }
    ...
    function _trySendCrashNotification(reason) {
        if (!_notifyTarget)
            return;
    ...
    async function drainThenExit(signal) {
        ...
        while (Date.now() < drainDeadline) {
            ...
            await Promise.all(pending.map(p => new Promise((resolve) => {
                p.on('close', () => resolve());
                p.on('error', () => resolve());
            })));
    ...
    async function handleMessage(context, message) {
        ...
        if (profile === 'twin' && (command === '/twin' || command.startsWith('/twin '))) {
            ...
            // /twin <content>：直接把内容走实体 /chat（实体内部自动 feed 记忆库），bot 只转发
            context.processedMessage = twinContent;
        }
        // ── /session 会话管理 ──
        ...
        if (STATUS_COMMANDS.has(command)) {
            ...
            function pidByPort(port) {
                try {
                    ...
                    if (start)
                        return { pid, uptimeS: (Date.now() - new Date(start).getTime()) / 1000 };
                }
                catch { }
                return null;
            }
            function _svc(state, name) {
            ...
            if (typeof channel.sendMarkdownCard === 'function') {
                await channel.sendMarkdownCard(context.conversationId, bodyParts.join('\n'), context.isGroup);
            }
            else {
            ...
            try {
                ...
                for (const [id, val] of entries) {
                    if (val.status === 'pending')
                        pending.push({ id, summary: val.summary });
                    else if (val.status === 'in_progress')
                        inProgress.push({ id, summary: val.summary, progress: val.progress });
                }
                ...
                if (inProgress.length > 0) {
                    ...
                    for (const t of inProgress) {
                        ...
                        if (t.progress)
                            lines.push(`    └ ${t.progress.slice(0, 50)}`);
                    }
                }
                if (pending.length > 0) {
                    lines.push(`\n**待处理 (${pending.length})**`);
        ...
        if (LOG_COMMANDS.has(command)) {
            ...
            try {
                ...
                for (const line of lines) {
                    ...
                    if (line.includes('Session Rotated')) {
                        rotates++;
                        continue;
                    }
                    // Extract timestamp
                    ...
                    if (line.includes('Received message from')) {
                        ...
                        if (line.includes('listening'))
                            mcpSample = `port ${line.match(/:(\d+)\/sse/)?.[1] || line.match(/localhost:(\d+)/)?.[1] || '?'}`;
                    }
                ...
                if (claudeCalls > 0)
                    parts.push(`Claude: ${claudeCalls} calls`);
                if (classifierCalls > 0)
                    parts.push(`Classifier: ${classifierCalls}`);
                if (mcpEvents > 0) {
                    let mcpStr = `MCP: ${mcpEvents} events${mcpSample ? ` (${mcpSample})` : ''}`;
                    if (mcpErr > 0)
                        mcpStr += ` | EADDRINUSE x${mcpErr}`;
                    parts.push(mcpStr);
                }
                if (cardSends > 0)
                    parts.push(`Cards: ${cardSends}`);
                if (errors.length > 0) {
        ...
        if (QUICK_ARCHIVE_COMMANDS.has(command)) {
            ...
            if (pair) {
                ...
                // 飞书通道发送确认卡片；其他通道直接存档
                if (typeof channel.sendCard === 'function') {
                    ...
                    await channel.sendCard(context.conversationId, cardBody, context.isGroup);
                }
                else {
        ...
        if (requiresApproval(risk.level)) {
            ...
            const approved = await new Promise((resolve) => {
                registerApproval(requestId, risk.level, strippedMessage, context.userId || context.senderId, context.conversationId, resolve);
                ...
                if (typeof channel.sendCard === 'function') {
                    ;
                    channel.sendCard(context.conversationId, cardBody, context.isGroup).catch((err) => {
                        logger(`[approval] sendCard failed: ${err.message}`);
                        resolve(false);
                    });
                }
                else {
                    channel.sendMessage(context.conversationId, `⚠️ **操作需要审批**但当前通道不支持交互卡片。\n请使用 /task run 手动提交此任务。`, context.isGroup);
                    resolve(false);
                }
            });
            if (!approved) {
        ...
        try {
            ...
            if (useStreaming) {
                ...
                if (profile === 'twin') {
                    // 实体是唯一大脑：bot 不判身份、不判模式、不拼上下文、不喂记忆，只传 channel+user_id
                    const pendingMsg = context.processedMessage ?? message;
                    let entityRes;
                    try {
                        entityRes = await chatWithEntity({
                        ...
                        // 实体是唯一路径（不下落本地区 spawn）；实体不可达时给个兜底提示，不发失控回复
                        logger(`[onMessage] entity /chat failed: ${entityErr}`);
                        await channel.sendMessage(context.conversationId, `分身服务暂不可用，请稍后再试。`, context.isGroup);
                        return;
                    }
                    let finalResult = entityRes.answer || '';
                    const finalThought = entityRes.thought || '';
                    // 身份由实体判定返回（owner / external）
                    const isOwner = entityRes.caller === 'owner';
                    const callerLevel = entityRes.caller === 'owner' ? 'owner' : 'external';
                    const callerName = entityRes.name || '';
                    const caller = entityRes.caller === 'owner' ? 'owner' : 'external';
                ...
                const { result: finalResult } = await callClaudeStreaming({
                    ...
                    switch (event.type) {
                        ...
                        case 'tool_use':
                            currentStepNumber++;
                            if (event.toolName)
                                collectedToolNames.push(event.toolName);
                            pendingStep = {
                        ...
                        case 'tool_result':
                            if (pendingStep) {
                                const summary = summarizeStep(pendingStep.number, pendingStep.toolName, pendingStep.toolInput);
    ...
    try {
        if (existsSync(daemonCrashMarker))
            unlinkSync(daemonCrashMarker);
    }
    catch { /* best-effort */ }
