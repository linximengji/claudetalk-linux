...
import { loadPeerMessages, removePeerMessages, writePeerMessagesFromContent, } from './peer-message.js';
import { appendChatHistory, loadChatHistory, formatChatHistory, } from './chat-history.js';
...
export class DingTalkClient {
     ...
     */
    async processPeerMessages() {
        const messages = loadPeerMessages(this.claudetalkDir, this.profileName);
        if (messages.length === 0)
            return;
        const now = Date.now();
        const pendingMessages = messages.filter((msg) => !this.processedPeerIds.has(msg.id) && now - msg.createdAt >= 0);
     ...
     */
    buildContextMessage(callback, messageText) {
    ...
    async connectStream() {
        ...
        return new Promise((resolve, reject) => {
            ...
            ws.onerror = (error) => {
                this.logger('DingTalk Stream WebSocket error: ' + (error instanceof Error ? error.message : String(error)));
                this.logger(`[ws.onerror] Error type: ${error?.type}`);
                this.logger(`[ws.onerror] Error message: ${error?.message}`);
    ...
    startReconnectLoop() {
        const attemptReconnect = async () => {
            try {
                ...
                this.logger(`[reconnect] Error details: ${error instanceof Error ? error.message : String(error)}`);
                // 指数退避，最大 60 秒
     ...
     * 处理 Stream 帧
     */
    async handleStreamFrame(ws, frame) {
    ...
    loadChatMembersConfig() {
        try {
            if (fs.existsSync(this.chatMembersPath)) {
                return JSON.parse(fs.readFileSync(this.chatMembersPath, 'utf-8'));
            }
        }
        catch {
            // 读取失败则返回空对象
     ...
     */
    updateChatMemberFromCallback(callback) {
        if (callback.conversationType !== '2')
            return; // 只处理群聊
        const conversationId = callback.conversationId;
        const config = this.loadChatMembersConfig();
        const members = config[conversationId] || [];
        ...
        if (existingUser) {
            ...
            if (callback.senderStaffId)
                newUser.staffId = callback.senderStaffId;
     ...
     */
    loadKnownProfilesFromChatMembers() {
        const config = this.loadChatMembersConfig();
        const botSelf = config['_bot_self'] || [];
        return botSelf
     ...
     */
    readAgentDisplayName(profileName) {
    ...
    async handleInboundMessage(callback) {
    ...
    async sendMessage(conversationId, content, isGroup) {
        ...
        if (isGroup) {
            ...
            for (const profile of knownProfiles) {
                const entry = botSelf.find((m) => m.profileName === profile);
                const displayName = entry?.name || this.readAgentDisplayName(profile);
                displayContent = displayContent.replace(new RegExp(`@${profile}\\b`, 'g'), `<at id=${profile}>${displayName}</at>`);
            }
        }
        ...
        if (isGroup) {
            ...
            const knownProfiles = this.loadKnownProfilesFromChatMembers();
            if (knownProfiles.length > 0) {
                writePeerMessagesFromContent(this.claudetalkDir, conversationId, content, this.profileName, knownProfiles);
            }
        }
    }
    /**
     * 发送单聊消息
     */
    async sendPrivateMessage(userId, content, msgKey = 'text') {
        const accessToken = await this.getAccessToken();
     ...
     * 发送群聊消息
     */
    async sendGroupMessage(conversationId, content, msgKey = 'text') {
        const accessToken = await this.getAccessToken();
        const robotCode = this.config.robotCode || this.config.clientId;
     ...
     */
    async sendMarkdownMessage(conversationId, content, isGroup) {
        const accessToken = await this.getAccessToken();
        const robotCode = this.config.robotCode || this.config.clientId;
     ...
     * 创建并投放 AI 卡片
     */
    async createAICard(conversationId, content) {
        const accessToken = await this.getAccessToken();
        const robotCode = this.config.robotCode || this.config.clientId;
        ...
        const response = await fetch(`${DINGTALK_API_BASE}/v1.0/card/instances/createAndDeliver`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-acs-dingtalk-access-token': accessToken,
     ...
     * 流式更新 AI 卡片
     */
    async streamAICard(card, content, isFinalize = false) {
        const accessToken = await this.getAccessToken();
        const templateKey = this.config.cardTemplateKey || 'content';
        const request = {
        ...
        const response = await fetch(`${DINGTALK_API_BASE}/v1.0/card/streaming`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'x-acs-dingtalk-access-token': accessToken,
     ...
     * 下载媒体文件
     */
    async downloadMedia(downloadCode) {
        const accessToken = await this.getAccessToken();
        const robotCode = this.config.robotCode || this.config.clientId;
        const response = await fetch(`${DINGTALK_API_BASE}/v1.0/robot/media/download?downloadCode=${downloadCode}&robotCode=${robotCode}`, {
            headers: {
                'x-acs-dingtalk-access-token': accessToken,
            },
        });
     ...
     * 发送媒体消息
     */
    async sendMediaMessage(conversationId, mediaBuffer, mediaType, fileName, isGroup) {
        const accessToken = await this.getAccessToken();
        const robotCode = this.config.robotCode || this.config.clientId;
        // 1. 上传媒体文件
        ...
        const uploadResponse = await fetch(`${DINGTALK_API_BASE}/v1.0/robot/media/upload?robotCode=${robotCode}`, {
            method: 'POST',
            headers: {
                'x-acs-dingtalk-access-token': accessToken,
            },
        ...
        if (isGroup) {
            return this.sendGroupMessage(conversationId, JSON.stringify(msgParam), msgKeyMap[mediaType]);
        }
        else {
            return this.sendPrivateMessage(conversationId, JSON.stringify(msgParam), msgKeyMap[mediaType]);
        }
    }
}
// ========== Channel 自注册 ==========
...
//# sourceMappingURL=index_dingtalk.js.map