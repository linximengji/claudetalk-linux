 ...
 */
export function getChatHistoryFilePath(claudetalkDir, conversationId) {
 ...
 * 读取指定会话的历史记录
 */
export function loadChatHistory(claudetalkDir, conversationId) {
    ...
    try {
        ...
        logger(`[chat-history] Failed to load history for ${conversationId}: ${error instanceof Error ? error.message : String(error)}`);
    }
    return [];
}
/**
 ...
 */
function atomicWriteChatHistory(claudetalkDir, conversationId, entries) {
    const filePath = getChatHistoryFilePath(claudetalkDir, conversationId);
 ...
 */
export function appendChatHistory(claudetalkDir, conversationId, entry) {
    ...
    if (entries.length > MAX_HISTORY_SIZE) {
        ...
        logger(`[chat-history] History for ${conversationId} exceeded ${MAX_HISTORY_SIZE}, removed ${removeCount} oldest entries`);
    }
    atomicWriteChatHistory(claudetalkDir, conversationId, entries);
    logger(`[chat-history] Appended ${entry.role} message to history_${conversationId}: senderId=${entry.senderId}, content=${entry.content.substring(0, 50)}`);
}
/**
 * 将历史记录格式化为可读文本，供注入到 Claude 上下文中
 * 格式：[时间] 角色(发送者): 内容
 */
export function formatChatHistory(entries) {
    if (entries.length === 0)
        return '';
