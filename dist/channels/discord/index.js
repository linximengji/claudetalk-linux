...
export class DiscordClient {
    ...
    async start() {
        ...
        try {
            ...
            if (isTimeout) {
                throw new Error('无法连接到 Discord 服务器（连接超时）。原因可能是：\n' +
            ...
            // 检查是否为 403 错误（网络被阻止）
            const isNetworkBlocked = error &&
                typeof error === 'object' &&
                'status' in error &&
                error.status === 403;
            if (isNetworkBlocked) {
                throw new Error('无法连接到 Discord 服务器（HTTP 403）。原因可能是：\n' +
                    '  1. 网络防火墙阻止了对 Discord 的访问\n' +
            ...
            if (isAuthError) {
                throw new Error('Discord Bot Token 无效或已过期，请前往 https://discord.com/developers/applications 重新生成 Token 并更新配置');
            }
            throw error;
        }
    }
     ...
     * 超过 2000 字符时自动分段发送
     */
    async sendMessage(conversationId, content, _isGroup) {
     ...
     * 可在新建 session 时注入上下文
     */
    async getHistoryMessages(channelId, limit = 10) {
