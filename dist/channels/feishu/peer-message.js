 ...
 */
export function getPeerMessageFilePath(claudetalkDir, botName) {
 ...
 */
export function loadPeerMessages(claudetalkDir, botName) {
    ...
    try {
        ...
        logger(`[peer-message] Failed to load bot_${botName}.json: ${error instanceof Error ? error.message : String(error)}`);
    }
    return [];
}
/**
 ...
 */
function atomicWritePeerMessages(claudetalkDir, botName, messages) {
    const filePath = getPeerMessageFilePath(claudetalkDir, botName);
 ...
 * 使用原子写入防止并发覆盖
 */
export function appendPeerMessage(claudetalkDir, botName, message) {
 ...
 * 使用原子写入防止并发覆盖
 */
export function removePeerMessages(claudetalkDir, botName, processedIds) {
    const existingMessages = loadPeerMessages(claudetalkDir, botName);
 ...
 */
export function parseAtMentions(content) {
...
export function writePeerMessagesFromContent(claudetalkDir, chatId, messageId, content, fromProfile, chatMembers) {
    ...
    for (const mention of mentions) {
        ...
        let matchedBot = chatMembers.find((member) => member.type === 'bot' && member.appId === mention.userId);
        // 第二优先：appId 匹配失败时，fallback 到 name 匹配（容错 at_id 填错的情况）
        if (!matchedBot) {
            matchedBot = chatMembers.find((member) => member.type === 'bot' && member.name === mention.name);
            if (matchedBot) {
        ...
        // 防止循环：自己不给自己写 peer-message
        if (botName === fromProfile) {
            logger(`[peer-message] Skip self-message: botName=${botName} === fromProfile=${fromProfile}`);
            continue;
        }
        const peerMessage = {
            id: randomUUID(),
