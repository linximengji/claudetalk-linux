...
const approvalLog = (msg) => log(`[approval] ${msg}`);
// ── 持久化 ──
function approvalDbPath() {
...
function addPersisted(p) {
    const map = loadPersisted();
    map.set(p.requestId, p);
    savePersisted(map);
}
function removePersisted(requestId) {
    const map = loadPersisted();
...
function finalizeApproval(requestId, approved, reason) {
...
export function registerApproval(requestId, riskLevel, messageSummary, requesterId, conversationId, resolve, timeoutMs = 180_000) {
...
export function handleApprovalCallback(value) {
    const requestId = value?.request_id || '';
    const decision = value?.decision || '';
    const clickerId = value?.clicker_id || '';
    ...
    if (!req) {
        ...
        const msg = p
            ...
            : '审批请求已过期或不存在';
        const entries = [...pendingApprovals.entries()].map(([id, r]) => `${id}(age=${now - r.createdAt}ms,requester=${r.requesterId},conv=${r.conversationId})`).join('; ');
...
export function inferTaskType(message) {
    if (/bug|故障|错误|闪退|crash|崩溃|异常|报错/i.test(message))
        return 'bug';
    if (/feature|功能|需求|新增|添加|支持/i.test(message))
        return 'feature';
    if (/调研|研究|分析|调查|research/i.test(message))
        return 'research';
    if (/配置|部署|安装|迁移|升级|chore/i.test(message))
        return 'chore';
    return 'task';
}
/** 根据消息关键词推断优先级 */
export function inferTaskPriority(message) {
    if (/紧急|urgent|critical|暂停|停用|立刻/i.test(message))
        return 'critical';
    if (/重要|important|high|严重|crash|闪退/i.test(message))
        return 'high';
    return 'medium';
}
/** 构建存档确认卡片 */
export function buildArchiveConfirmCard(messagePreview) {
    return JSON.stringify({
        config: { wide_screen_mode: true },
        header: {
