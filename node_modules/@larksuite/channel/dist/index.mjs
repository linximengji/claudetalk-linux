...
import { Client, Domain, EventDispatcher, LoggerLevel, LoggerProxy, WSClient, defaultHttpInstance, defaultLogger, internalCache, registerApp } from "@larksuiteoapi/node-sdk";
	...
	async toBuffer(source) {
		...
		if (!/^https?:\/\//i.test(source)) {
			...
			try {
				...
				}))).some((d) => realPath === d || realPath.startsWith(d + path.sep))) throw new LarkChannelError("upload_failed", `file path is outside allowed directories: ${realPath}`);
...
var SafetyPipeline = class {
	...
	async pushMessage(msg) {
		console.error('[channel-debug] pushMessage called, messageId=' + msg.messageId + ' chatId=' + msg.chatId);
...
var LarkChannel = class {
	...
	constructor(opts) {
		this.opts = opts;
		this.logger = new LoggerProxy(opts.loggerLevel ?? LoggerLevel.info, opts.logger ?? defaultLogger);
...
//#endregion
export { CommentSurface, LarkChannel, LarkChannelError, createLarkChannel, normalize, normalizeBotAdded, normalizeCardAction, normalizeComment, normalizeReaction, registerApp };
